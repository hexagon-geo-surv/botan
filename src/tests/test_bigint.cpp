...
class BigInt_InvMod_Test final : public Text_Based_Test {
   public:
      ...
      Test::Result run_one_test(const std::string& /*header*/, const VarMap& vars) override {
         ...
         if(a < mod && a > 0 && a < mod) {
            auto g = Botan::inverse_modulo_general(a, mod);
            if(g.has_value()) {
               result.test_eq("inverse_modulo_general", g.value(), expected);
               result.test_eq("inverse works", ((g.value() * a) % mod), BigInt::one());
            } else {
               result.confirm("inverse_modulo_general", expected.is_zero());
            }

            if(Botan::is_prime(mod, rng()) && mod != 2) {
               BOTAN_ASSERT_NOMSG(expected > 0);
               result.test_eq("inverse_modulo_secret_prime", Botan::inverse_modulo_secret_prime(a, mod), expected);

               // Currently inverse_modulo_public_prime just forwards to inverse_modulo_secret_prime so we'll
               // skip this test until a specific algorithm is implemented for this function
               //result.test_eq("inverse_modulo_public_prime", Botan::inverse_modulo_public_prime(a, mod), expected);
            }
         }
