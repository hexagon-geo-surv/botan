...
X509_CA::X509_CA(const X509_Certificate& cert,
   ...
   if(padding_method == "")
      {
      // by default use the padding used in the CA certificate
      // sig_oid_str = <sig_alg>/<padding>, so padding with all its options will look
      // like a cipher mode to the scanner
      const std::string sig_oid_str = OIDS::oid2str_or_throw(m_ca_cert.signature_algorithm().get_oid());
      SCAN_Name scanner(sig_oid_str);
      const std::string cert_padding = scanner.cipher_mode();
      m_signer = X509_Object::choose_sig_format(m_ca_sig_algo, key, rng, hash_fn, cert_padding);
      }
   else
      {
