...
bool Server_Impl_13::new_session_ticket_supported() const {
   ...
   return is_handshake_complete() && m_active_state->peer_supports_psk_dhe_ke();
}
...
size_t Server_Impl_13::send_new_session_tickets(const size_t tickets) {
   ...
   for(size_t i = 0; i < tickets; ++i) {
      ...
      const auto ticket_age_add = load_be<uint32_t>(rng().random_vec(4).data(), 0);
...
void Server_Impl_13::handle_reply_to_client_hello(Server_Hello_13 server_hello) {
   ...
   flight.add(m_handshake->state.sending(Encrypted_Extensions(client_hello, policy(), callbacks())));
...
void Server_Impl_13::handle(const Finished_13& finished_msg) {
   ...
   {
      auto certs = [&]() -> std::vector<X509_Certificate> {
         if(m_handshake->state.has_client_certificate_msg() &&
            m_handshake->state.client_certificate().has_certificate_chain()) {
            return m_handshake->state.client_certificate().cert_chain();
         }
      ...
      auto raw_pk = [&]() -> std::shared_ptr<const Public_Key> {
         if(m_handshake->state.has_client_certificate_msg() &&
            m_handshake->state.client_certificate().is_raw_public_key()) {
            return m_handshake->state.client_certificate().public_key();
         }
      ...
      const auto& eee = m_handshake->state.encrypted_extensions().extensions();
      auto alpn = eee.get<Application_Layer_Protocol_Notification>() != nullptr
                     ? eee.get<Application_Layer_Protocol_Notification>()->single_protocol()
                     : std::string{};
      ...
      m_active_state = Active_Connection_State_13(m_handshake->state.server_hello().selected_version(),
                                                  m_handshake->state.server_hello().ciphersuite(),
                                                  m_handshake->state.client_hello().random(),
                                                  std::move(alpn),
                                                  std::move(certs),
                                                  std::move(raw_pk),
                                                  m_handshake->psk_identity,
