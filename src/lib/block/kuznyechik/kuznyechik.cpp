...
constexpr auto sqr_matrix(std::span<const uint8_t, 256> mat) {
...
constexpr std::array<uint64_t, 16 * 256 * 2> T_table() {
   std::array<uint8_t, 256> l_matrix = {};
   ...
   for(size_t i = 0; i != 16; ++i) {
      ...
      for(size_t j = 0; j != 8; ++j) {
         L_stride_0 |= static_cast<uint64_t>(l_matrix[i + 16 * j]) << (8 * (j % 8));
         L_stride_1 |= static_cast<uint64_t>(l_matrix[i + 16 * (j + 8)]) << (8 * (j % 8));
      }

      for(size_t j = 0; j != 256; ++j) {
         const uint8_t Sj = S[j];
...
constexpr std::array<uint64_t, 16 * 256 * 2> IT_table() {
   std::array<uint8_t, 256> l_matrix = {};

   for(size_t i = 0; i != 16; ++i) {
      l_matrix[16 * 15 + i] = LINEAR[(i + 15) % 16];
      if(i < 15) {
         l_matrix[17 * i + 1] = 1;
      }
   }
   ...
   for(size_t i = 0; i != 16; ++i) {
      ...
      for(size_t j = 0; j != 256; ++j) {
         const uint8_t ISj = IS[j];
         T[512 * i + 2 * j] = poly_mul(L_stride_0, ISj);
         T[512 * i + 2 * j + 1] = poly_mul(L_stride_1, ISj);
      }
   }
...
const constinit auto T = T_table();
const constinit auto IT = IT_table();
