...
class SIMD_16x16 final {
   public:
      ...
      static SIMD_16x16 BOTAN_FN_ISA_AVX2 load(const uint8_t in[]) {
         return SIMD_16x16(_mm256_loadu_si256(reinterpret_cast<const __m256i*>(in)));
      }

      void BOTAN_FN_ISA_AVX2 store(uint8_t out[]) const {
      ...
      static inline BOTAN_FN_ISA_AVX2 SIMD_16x16 mul_mod_65537(SIMD_16x16 X, uint16_t K_16) {
         ...
         // Each u16 be set to all-1 mask if == 0
...
/*
* 16 wide IDEA encryption/decryption in AVX2
*/
BOTAN_FN_ISA_AVX2 void IDEA::avx2_idea_op_16(const uint8_t in[128], uint8_t out[128], const uint16_t EK[52]) {
   ...
   auto B0 = SIMD_16x16::load(in + 0);
   auto B1 = SIMD_16x16::load(in + 32);
   auto B2 = SIMD_16x16::load(in + 64);
   auto B3 = SIMD_16x16::load(in + 96);
