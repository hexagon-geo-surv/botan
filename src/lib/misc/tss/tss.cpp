...
secure_vector<uint8_t> RTSS_Share::reconstruct(const std::vector<RTSS_Share>& shares) {
   ...
   for(size_t k = 0; k != shares.size(); ++k) {
      ...
      for(size_t l = 0; l != shares.size(); ++l) {
         ...
         if(share_k == 0 || share_l == 0) {
            // Should have already been checked during deserialization
            throw Decoding_Error("Invalid RTSS share ID");
         }
         const uint8_t div = tss_gf_mul(share_l, gfp_inv(share_k ^ share_l));
