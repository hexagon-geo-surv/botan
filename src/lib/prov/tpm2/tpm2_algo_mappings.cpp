...
std::optional<TPMT_SYM_DEF> cipher_botan_to_tss2(std::string_view algo_name) {
   ...
   const auto m = spec.match("CFB|CBC|ECB|OFB|CTR|CTR-BE({cipher})");
   if(!m) {
      return std::nullopt;
   }
...
std::optional<TPMT_SIG_SCHEME> rsa_signature_scheme_botan_to_tss2(std::string_view name) {
   ...
   const auto scheme_for = [](const AlgorithmMatch& m, TPMI_ALG_SIG_SCHEME scheme) -> std::optional<TPMT_SIG_SCHEME> {
   ...
   // TODO(Botan4) remove the deprecated aliases
   if(const auto m = req.match("PKCS1v15|EMSA_PKCS1|EMSA-PKCS1-v1_5|EMSA3({hash})")) {
      return scheme_for(*m, TPM2_ALG_RSASSA);
   }

   // PSS with an explicit MGF1 or salt size is not expressible
   if(const auto m = req.match("PSS|PSSR|EMSA-PSS|PSS-MGF1|EMSA4({hash})")) {
      return scheme_for(*m, TPM2_ALG_RSAPSS);
   }
