...
/**
* If c is zero, returns zero. Otherwise returns the polynomial
*/
template <MinWeightPolynomial P>
inline uint64_t return_carry(uint64_t c) {
   return CT::Mask<uint64_t>::expand(c).if_set_return(static_cast<uint64_t>(P));
}
...
void poly_double(uint8_t out[], const uint8_t in[]) {
   ...
   const uint64_t carry = return_carry<P>(W[0] >> 63);
...
void poly_double_le(uint8_t out[], const uint8_t in[]) {
   ...
   const uint64_t carry = return_carry<P>(W[LIMBS - 1] >> 63);
...
void xts_update_tweak_block(uint8_t tweak[], size_t BS, size_t blocks_in_tweak) {
   if(BS == 16) {
      ...
      for(size_t i = 1; i < blocks_in_tweak; ++i) {
         const uint64_t carry = return_carry<MinWeightPolynomial::P128>(W[1] >> 63);
         W[1] = (W[1] << 1) ^ (W[0] >> 63);
         W[0] = (W[0] << 1) ^ carry;
