...
std::shared_ptr<Atomic<size_t>> XMSS_Index_Registry::get(uint32_t params,
   ...
   BOTAN_ASSERT_NOMSG(m_key_ids.size() == m_leaf_indices.size());

   for(size_t i = 0; i < m_key_ids.size(); i++) {
      if(m_key_ids[i] == id) {
         return m_leaf_indices[i];
      }
   }

   m_key_ids.push_back(id);
   m_leaf_indices.push_back(std::make_shared<Atomic<size_t>>(0));
   return m_leaf_indices.back();
}
