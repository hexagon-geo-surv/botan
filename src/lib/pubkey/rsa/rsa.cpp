...
RSA_PrivateKey::RSA_PrivateKey(
   ...
   BigInt c = inverse_modulo_secret_prime(ct_modulo(q, p), p);
...
bool RSA_PrivateKey::check_key(RandomNumberGenerator& rng, bool strong) const {
   ...
   if(get_c() != inverse_modulo_secret_prime(ct_modulo(get_q(), get_p()), get_p())) {
      return false;
   }
   ...
   protected:
      ...
      explicit RSA_Private_Operation(const RSA_PrivateKey& rsa, RandomNumberGenerator& rng) :
            ...
            m_blinder(
               ...
               [this](const BigInt& k) { return inverse_modulo_rsa_public_modulus(k, m_public->get_n()); }),
            m_blinding_bits(64),
