...
class Kyber_Symmetric_Primitives /* NOLINT(*-special-member-functions) */ {
   public:
      ...
      /// Creates a new seeded XOF for matrix sampling
      std::unique_ptr<Botan::XOF> XOF(StrongSpan<const KyberSeedRho> seed,
                                      std::tuple<uint8_t, uint8_t> matrix_position) const {
         return create_XOF(seed, matrix_position);
      }

      /// Reinitializes an existing matrix sampling XOF
      void reseed_XOF(Botan::XOF& xof,
                      StrongSpan<const KyberSeedRho> seed,
                      std::tuple<uint8_t, uint8_t> matrix_position) const {
         init_XOF(xof, seed, matrix_position);
      }

      /// Creates a new seeded PRF XOF for polynomial sampling
      std::unique_ptr<Botan::XOF> create_seeded_PRF(std::span<const uint8_t> seed, uint8_t nonce) const {
         return create_PRF(seed, nonce);
      }

      /// Reinitializes an existing PRF XOF
      void reseed_PRF(Botan::XOF& xof, std::span<const uint8_t> seed, uint8_t nonce) const {
