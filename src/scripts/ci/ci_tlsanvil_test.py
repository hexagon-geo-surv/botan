...
def server_test(botan_cli: str, parallel: int):
    ...
    botan_server_cmd = [
        botan_cli, "tls_http_server", cert_path, key_path, f"--port={Config.server_dest_port}", "--policy=anvil",
    ]
...
def client_test(botan_cli: str, parallel: int):
    ...
    trigger_server_script = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "ci_tlsanvil_trigger_server.py")
