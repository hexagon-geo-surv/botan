...
class PK_Signature_Command : public Command {
   protected:
      ...
      Botan::PK_Signature_Options signature_options(const Botan::Public_Key& key) const {
         ...
         if(hash_fn.empty() && signature_scheme_requires_hash(algo_name)) {
            hash_fn = "SHA-256";
         }
         if(!hash_fn.empty() && hash_fn != "Raw" && !Botan::HashFunction::create(hash_fn)) {
   ...
   public:
      PK_Sign() :
            PK_Signature_Command(
               "sign --der-format --passphrase= --hash= --padding= --prehash= --context= --salt-size= --deterministic "
               "--provider= --rng-type= --drbg-seed= key file") {}
...
class PK_Verify final : public PK_Signature_Command {
   public:
      PK_Verify() :
            PK_Signature_Command(
               "verify --der-format --hash= --padding= --prehash= --context= --salt-size= pubkey file signature") {}
